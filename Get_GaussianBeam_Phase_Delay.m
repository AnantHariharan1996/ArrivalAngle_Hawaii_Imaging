...
x_forgrid = zeros(size(GridLon));
R_forgrid = zeros(size(GridLon));
...
% must set this to zero- can't have acausal phase perturbation
idx = find(ev2grid_dist < alen);
tau(idx)=0;
Q(idx)=0;

%%%%
% commented the line below out for redundancy, AH 2025 
[grid_dist,gridaz] = distance(evla,evlo,GridLat,GridLon);
[grid_dist_km] = deg2km(grid_dist);

% linear increase with distance corresponding to plane wave.
