...
addpath(genpath(pwd))
Min_N_Stations = 3;
L_TOL = 3.5O; % UNITS OF DEGREES DUMBO
Periodlist = [50];
...
Pcounter=Pcounter+1;
GoodIDFname = [PredictionsDir 'WIDE_IDLIST_' num2str(Period) 's.txt'];
...
Foster_Angle_Resid = ArrivalAngleList' - griddata(xgrid,ygrid,angle,Slon,Slat);
        ...
        scatter(ax(evnum),Slon,Slat,200,Foster_Angle_Resid,'filled','MarkerEdgeColor','k','LineWidth',2)
...
text(ax(evnum),-158, 28,['Event ' num2str(EVID)],'fontsize',16,'FontWeight','bold')

set(ax(evnum),'linewidth',2,'box','on','XTickLabel',[],'YTickLabel',[])
   ax(evnum).XLim=[-162 -150];
     ax(evnum).YLim=[12 30];
ax(evnum).CLim = [-5 5];
...
text(ax(eventnum),0.75,0.5,[num2str(Period) ' s'],'fontsize',24,'fontweight','bold')

set(junk2,'position',[1637 -77 1134 956])
saveas(junk2,[SummaryFigDir 'NoSurface_WIDE_OurObsFosterOverlain' num2str(Period) 's' '.png'])
 %close all
