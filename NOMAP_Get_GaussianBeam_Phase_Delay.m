function [ttime_field_perturbed,tau,ttime_field_noscatter,R_forgrid,x_forgrid,Q] = ...
    NOMAP_Get_GaussianBeam_Phase_Delay(Period,evla,evlo,...
...
% Need to preallocate X and R here. 
x_forgrid = zeros([1 length(GridLon)]);
R_forgrid = x_forgrid;
for ijk = 1:length(GridLon)  
    distlist = distance_deg_nomap(tracklat,tracklon,GridLat(ijk),GridLon(ijk));
